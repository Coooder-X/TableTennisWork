...
def notProgress(fileNameList):
    ...
    for fileName in fileNameList:
        ...
        #   得到统计对象的TeamID（str类型）
        TargetTeamPair = dUts.getTargetTeamID(playerList)

        roundList = data['record']['list']  # 若干局，元素是每个个大比分 局内的的信息

        hitOrders = dUts.getHitOrders(playerList)
        print(hitOrders)
        ...
        count = 0
        # print(dUts.getHitOrders(playerList))
        ...
        for roundId, round in enumerate(roundList):  # 其中的一局的信息
            ...
            for pointId, point in enumerate(pointList):
                ...
                if len(hitOrders) == 2:
                    playerPair = ['00', '10']
                    A1 = playerPair[dUts.getTargetPlayer(playerPair, TargetTeamPair[0])]  # 当前pair中中国运动员
                    B1 = playerPair[dUts.getOppositePlayer(playerPair, TargetTeamPair[0])]  # 当前pair对方运动员

                    if rallyNum == 1:  # 发球失误的情况特判一下
                        if rallyList[0]['HitPlayer'] == A1:
                            print('发球失误', A1, B1)
                            index = hitOrders.index([A1, B1])
                            dUts.updateScoreCase(scoreCase, index, 0, 0, 'lost')
                            dUts.updateScoreCase(scoreCase, index, 1, 1, 'win')
                    ...
                    if servePair[0] == lastHitPair[0]:  # 发球的赢了
                        dUts.updateScoreCase(scoreCase, index, 0, rallyNum - 2, 'win')
                        dUts.updateScoreCase(scoreCase, another, 1, rallyNum - 1, 'lost')
                    else:
                        dUts.updateScoreCase(scoreCase, index, 0, rallyNum - 1, 'lost')
                        dUts.updateScoreCase(scoreCase, another, 1, rallyNum - 2, 'win')
                    continue

                #   双打情况--------------------------------------------------
                if roundId == 0 and pointId == 0:
                    dUts.updateServeRecOrder(serve_rec_order, rallyList[0]['HitPlayer'],
                ...
                dUts.updateDoublesData(scoreCase, rallyList, hitOrders, TargetTeamPair, serve_rec_order, False,
                                              cnt, cnt2)
                dUts.updateDoublesData(scoreCaseOpposite, rallyList, hitOrders, TargetTeamPair, serve_rec_order,
                                              True,
                                              cnt, cnt2)
        ...
        calScoreOpposite = {}
        count = 0
        MaxScore = dUts.getExcelScoreCase(excelScoreCase, scoreCase, calScore, hitOrders, playerList, False)
        MaxScoreOpposite = dUts.getExcelScoreCase(excelScoreCaseOpposite, scoreCaseOpposite,
                                                         calScoreOpposite, hitOrders, playerList, True)

        # ----------------- 每局下方不同轮次的得分情况、得分率统计 ------------------
        dUts.getAdvanceRate(excelScoreCase, scoreCase, calScore, hitOrders, playerList)
        dUts.getAdvanceRate(excelScoreCaseOpposite, scoreCaseOpposite, calScoreOpposite, hitOrders, playerList)
        # print(count)
        ...
        tmpLine = nowLine
        nowLine = dUts.fillExcel(excelScoreCase, calScore, tmpLine, baseColumn, sheet, fileName, MaxScore)
        ...
        for fileName in fileNameList:
            ...
            for roundId, round in enumerate(roundList):  # 其中的一局的信息
                ...
                for pointId, point in enumerate(pointList):
                    ...
                    dUts.updateDoublesData(scoreCase, rallyList, hitOrders, TargetTeamPair, serve_rec_order,
                                                  False,
                                                  cnt, cnt2)
                    dUts.updateDoublesData(scoreCaseOpposite, rallyList, hitOrders, TargetTeamPair,
                                                  serve_rec_order,
                                                  True,
                                                  cnt, cnt2)
            ...
            # ----------------- 每局下方不同轮次的得分情况、得分率统计 ------------------
            dUts.getAdvanceRate(excelScoreCase, scoreCase, calScore, hitOrders, playerList)
            dUts.getAdvanceRate(excelScoreCaseOpposite, scoreCaseOpposite, calScoreOpposite, hitOrders,
                                       playerList)
            # print(count)
            ...
            for roundId, round in enumerate(roundList):  # 其中的一局的信息
                ...
                for pointId, point in enumerate(pointList):
                    ...
                    if gameProgress[nowScore[0]][nowScore[1]] != gameProgress[tmp[0]][tmp[1]]:
                        print('临界得分：', '局数：', roundId + 1, tmp, nowScore)
                    # print(nowScore)
                    teamIdPair = list(map(int, dUts.getTargetTeamID(playerList)))
