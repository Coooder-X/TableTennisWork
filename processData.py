...
with open('20211128 休斯顿世乒赛 女单半决赛 陈梦vs王曼昱-collect_project(new).json', 'r', encoding='utf-8') as f:
    jsonFile = f.read()
...
for round in roundList: # 其中的一局的信息
    ...
    for point in pointList: #   point 记录该分中的信息
        ...
        if rallyList[-2]['index'] > 3:
            utils.updateLineScoreCase(lineScoreCaseDict, rallyList, playerList[0][0], playerList[1][0], serveSide, winSide)
